...
As of the current session, `package.json` also records a `mountedOn` field for a mechanism that's rigidly attached to another mechanism's flange (e.g. a gripper mounted on the robot's tool0, ~114mm offset along flange Z) -- `ExportPipeline.FindMechanismMounts` detects this via `AttachmentHelper.GetAttachedMechanism`, a real SDK query over the station's `Attachments` list (not a heuristic), and resolves which link index of the parent mechanism the flange corresponds to. The Unity importer (`RobotPackageLoader`) re-parents that mechanism's root under the resolved link transform instead of the package root, so it rides along with the arm's existing per-link animation automatically -- no separate timeline entries needed for the gripper's own position, only for its finger joints if it has any. Not yet verified against a live RobotStudio 2025 recording (only compiled against the real SDK); still missing before this is a real hand-off-able deliverable: deciding whether the package should be zipped; end-to-end verification that a picked-up static component's motion round-trips correctly through a real RobotStudio 2025 recording + export + Unity playback.
